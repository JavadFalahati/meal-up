package com.jwdfhi.meal_up.screens.mark
...
class MarkViewModel @Inject constructor(
    val mealDatabaseRepository: MealDatabaseRepository
    ...
    private fun getMarkedMeals() {
        viewModelScope.launch(Dispatchers.IO) {
            _mealsDataOrException.value = DataOrException(status = DataOrExceptionStatus.Loading)

            _storedMealList = mealDatabaseRepository.getMeals().filter { storedMeal -> storedMeal.isMarked }
